...
app.MapGet("/", async (HttpContext httpContext, ShareManager manager, CancellationToken ct) =>
{
    if (httpContext.User.Identity?.IsAuthenticated != true)
    {
...
<section class="mb-10 rounded-2xl border border-border bg-white p-8">
  ...
  <p class="text-ink-muted text-sm mt-4 max-w-xl">Create expiring share links with customizable landing pages, download notifications, and background processing built in.</p>
  {(shares.Count > 0
    ? """
  <div class="mt-6">
...
app.MapGet("/shares/new", async (HttpContext httpContext, AuthService authService, ShareManager manager, CancellationToken ct) =>
{
    if (httpContext.User.Identity?.IsAuthenticated != true)
    {
        return Results.Redirect("/login");
...
app.MapGet("/account/landing-page-designer", async (HttpContext httpContext, ShareManager manager, CancellationToken ct) =>
{
    if (httpContext.User.Identity?.IsAuthenticated != true)
    {
        return Results.Redirect("/login");
    ...
    return Results.Content(RenderLayout("Landing Page Settings", email, body, msg, isAdmin), "text/html");
}).RequireAuthorization();

app.MapGet("/account/settings", async (HttpContext httpContext, AuthService authService, CancellationToken ct) =>
{
    if (httpContext.User.Identity?.IsAuthenticated != true)
    {
        return Results.Redirect("/login");
...
app.MapGet("/share/landing-page-designer", async (HttpContext httpContext, ShareManager manager, CancellationToken ct) =>
{
    if (httpContext.User.Identity?.IsAuthenticated != true)
    {
        return Results.Redirect("/login");
    ...
    return Results.Content(RenderLayout("Share Landing Page Designer", email, body, msg, isAdmin), "text/html");
}).RequireAuthorization();
...
app.MapGet("/s/{token}/background", async (ShareManager manager, IOptions<AgoraOptions> options, string token, CancellationToken ct) =>
{
    var share = await manager.FindByTokenAsync(token, ct);
    if (share is null)
    {
...
"""))}
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label class="text-xs font-medium text-ink-muted uppercase tracking-wider mb-1.5 block">Landing page design</label>
...
<section class="mb-8">
  ...
  <p class="text-ink-muted text-sm mt-1">Manage share defaults and account-wide landing page settings.</p>
</section>
...
<section id="landing-page-settings" class="scroll-mt-24">
  <div class="bg-white rounded-2xl border border-border p-6">
    <h3 class="font-display text-xl mb-2">Landing page settings</h3>
    <p class="text-sm text-ink-light">Manage the account-wide landing page template and background image.</p>
    <a href="/account/landing-page-designer" class="inline-block mt-4 px-4 py-2 bg-ink text-white text-sm font-medium rounded-lg hover:bg-ink/90 transition-colors">Open landing page designer</a>
  </div>
</section>
""";
...
static string RenderAccountLandingPageDesignerBody(ShareTemplateData template)
{
    return $"""
<section class="mb-8">
  <h2 class="font-display text-3xl tracking-tight">Landing page settings</h2>
  <p class="text-ink-muted text-sm mt-1">Set your account-wide default landing page template.</p>
</section>
...
<section class="mb-8">
  <h2 class="font-display text-3xl tracking-tight">Share landing page designer</h2>
  <p class="text-ink-muted text-sm mt-1">Configure this share's landing page. Save and return to the upload form.</p>
</section>
<section class="space-y-6">
  <div class="bg-white rounded-2xl border border-border p-6">
  ...
  <details class="relative">
    <summary class="list-none cursor-pointer">
      <span class="inline-flex items-center gap-2 rounded-lg border border-border bg-white px-3 py-1.5 text-sm text-ink-light hover:border-terra/40">
        <span class="w-6 h-6 rounded-full bg-terra-wash text-terra text-xs font-medium flex items-center justify-center">{Html(email.Substring(0, 1).ToUpperInvariant())}</span>
        <span class="flex flex-col leading-tight">
          <span>Account</span>
          <span class="text-[11px] text-ink-muted">{Html(email)}</span>
        </span>
      </span>
    </summary>
    <div class="absolute right-0 mt-2 w-56 rounded-xl border border-border bg-white p-2 shadow-lg z-20">
      <a href="/account/settings#account-settings" class="block rounded-lg px-3 py-2 text-sm text-ink-light hover:bg-cream-dark/60">Account settings</a>
      <a href="/account/landing-page-designer" class="block rounded-lg px-3 py-2 text-sm text-ink-light hover:bg-cream-dark/60">Landing page settings</a>
      <a href="/account/settings#share-defaults" class="block rounded-lg px-3 py-2 text-sm text-ink-light hover:bg-cream-dark/60">Share defaults</a>
