...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  var Subject = class {
    ...
    subscribe(observer2) {
      ...
      return new SubjectSubscription(this, observer2);
    }
  };
  ...
  var TransportSendQueue = class _TransportSendQueue {
    ...
    async _sendLoop() {
      while (true) {
        ...
        const data2 = typeof this._buffer[0] === "string" ? this._buffer.join("") : _TransportSendQueue._concatBuffers(this._buffer);
  ...
  (() => {
    const page = document.querySelector("[data-share-created-page]");
    if (!page) return;
    ...
    if (!token) return;
    const statusText = page.querySelector("[data-created-status-text]");
    const spinner = page.querySelector("[data-created-spinner]");
    const errorNode = page.querySelector("[data-created-error]");
    const taskList = page.querySelector("[data-created-task-list]");
    const startedAt = Date.now();
    const maxMs = 5 * 60 * 1e3;
    const intervalMs = 15 * 1e3;
    let timer = null;
    let stopped = false;
    const normalizeState = (value) => (value || "").trim().toLowerCase();
    const renderTasks = (steps) => {
      if (!taskList) return;
      ...
      list.forEach((step) => {
        ...
        const dotClass = state === "completed" ? "bg-sage" : state === "active" ? "bg-terra animate-pulse" : state === "failed" ? "bg-danger" : "bg-ink-muted/40";
    ...
    const connectSignalR = async () => {
      ...
      connection.on("shareStatus", (payload) => {
  ...
  function runConfetti() {
    ...
    const tick = (timestamp) => {
      ...
      draw();
      if (timestamp - start2 <= durationMs) {
...
//# sourceMappingURL=shares-created.js.map
