...
| ~~High~~ | ~~Raise token strength + add rate limiting~~ | ~~Short token acceptance plus no throttling increases brute-force/enumeration risk.~~ | **Fixed in 0.9.1** — rate limiting added for auth, authenticated, and download endpoints. | Codex |
| ~~High~~ | ~~Add CSRF protection for cookie-authenticated POST APIs~~ | ~~Prevent cross-site state-changing requests against authenticated sessions.~~ | **Fixed in 0.9.1** — antiforgery validation on unsafe HTTP methods. | Claude + Codex |
| ~~High~~ | ~~Add login brute-force protections~~ | ~~Reduces credential stuffing/password guessing impact.~~ | **Fixed in 0.9.1** — temporary account lockout after repeated failures. | Claude + Codex |
